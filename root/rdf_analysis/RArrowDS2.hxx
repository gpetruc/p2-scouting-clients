...
private:
   ...
   virtual Record_t GetColumnReadersImpl(std::string_view /*name*/, const std::type_info &) override { return Record_t(); }
   virtual std::unique_ptr<ROOT::Detail::RDF::RColumnReaderBase> GetColumnReaders(unsigned int,
                                                                                  std::string_view,
                                                                                  const std::type_info &) override;

   void maybeAddColumns(const std::string &name, const arrow::DataType & field, ColumnAddress const & base, std::vector<std::string> const &requestedColumns) ;
